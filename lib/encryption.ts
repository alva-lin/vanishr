...
async function importKeyFromBase64(keyStr: string): Promise<CryptoKey> {
  ...
  return await window.crypto.subtle.importKey('raw', keyBuffer, { name: 'AES-GCM', length: 256 }, true, [
    'encrypt',
    'decrypt',
  ]);
}
...
// 加密文件
async function encryptFile(file: File): Promise<{ encryptedFile: Blob; key: string }> {
...
// 解密文件
async function decryptFile(encryptedBlob: Blob, keyBase64: string): Promise<Blob> {
  const key = await importKeyFromBase64(keyBase64);
  const encryptedBuffer = await encryptedBlob.arrayBuffer();
  const iv = encryptedBuffer.slice(0, 12);
  const data = encryptedBuffer.slice(12);

  const decryptedContent = await window.crypto.subtle.decrypt({ name: 'AES-GCM', iv: new Uint8Array(iv) }, key, data);
